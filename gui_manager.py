...
class ModernGUI:
    ...
    def _encrypt_folder_thread(self, folder_path, password):
        ...
        try:
            self.update_status("Encrypting folder...")
            self.add_info(f"Starting encryption of folder: {Path(folder_path).name}")
            ...
            self.add_info(f"Folder encrypted successfully: {folder_name}")
            self.update_status("Encryption completed successfully")

            messagebox.showinfo("Success", f"Folder '{folder_name}' has been locked successfully!")

        except Exception as e:
            self.add_info(f"Folder encryption failed: {str(e)}")
    ...
    def _decrypt_folder_thread(self, encrypted_file, password, folder_name):
        ...
        try:
            self.update_status("Unlocking folder...")
            self.add_info(f"Starting decryption of folder: {folder_name}")
            ...
            self.add_info(f"Folder unlocked successfully: {Path(restored_folder).name}")
            self.update_status("Unlock completed successfully")

            messagebox.showinfo("Success", f"Folder unlocked successfully!\nLocation: {restored_folder}")

        except Exception as e:
            self.add_info(f"Folder unlock failed: {str(e)}")
            self.update_status("Unlock failed")
            messagebox.showerror("Error", f"Failed to unlock folder:\n{str(e)}")
